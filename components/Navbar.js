  ...
  return React.createElement('nav', {
    ...
    React.createElement('div', { className: 'container mx-auto px-6 flex justify-between items-center' },
      React.createElement('div', { className: 'flex items-center gap-2' },
        React.createElement('div', { className: 'bg-indigo-700 p-2 rounded-lg shadow-indigo-200 shadow-lg' },
          React.createElement('i', { className: 'fas fa-gavel text-white text-lg' })
        ),
        React.createElement('span', { className: 'text-xl font-bold tracking-tight text-slate-900' }, 'LegalHelp')
      ),
      ...
      React.createElement('button', {
        ...
        React.createElement('i', { className: `fas ${isOpen ? 'fa-times' : 'fa-bars'} text-xl` })
      )
    ),
    isOpen && React.createElement('div', {
      className: 'md:hidden absolute top-full left-0 w-full bg-white border-b border-slate-100 py-8 px-6 flex flex-col gap-6 shadow-2xl animate-fade-in'
    },
      navLinks.map((link) =>
        React.createElement('a', {
          key: link.name,
